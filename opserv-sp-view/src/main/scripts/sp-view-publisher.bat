@echo off
rem This script runs the extract
rem Created by: Cognizant Technology Solutions 04/05/2014
Rem Environment : ${deploy.env} | Changeset : ${tfs.changeset} | Build on : ${build.timestamp}
...
echo. 
echo Starting SP View Publisher....

rem Set Java Opts
set "JAVA_OPTS=-DappType=standalone -DtargetDataSource=app -DappName=EntityChangeEventPublisher"
...
set command=%JAVA_CMD% %JAVA_OPTS% -cp %JAVA_CP% com.cognizant.opserv.sp.view.EntityChangeEventPublisher %--spId% %--entityId% %--entityType%
echo COMMAND USED : %command%
echo.
"%JAVA_CMD%" %JAVA_OPTS% -cp "%JAVA_CP%" com.cognizant.opserv.sp.view.EntityChangeEventPublisher %--spId% %--entityId% %--entityType%
