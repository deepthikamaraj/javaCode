 ...
 *        ***************
 */
@Component("changeRequestApproveListener")
@EnableTransactionManagement
public class ChangeRequestApproveListener implements MessageListener {

	@Autowired
	private ChangeRequestOfflineService changeRequestProcessInternalService;

	private static final OpservLogger LOGGER = OpservLoggerFactory.getOpservLogger(ChangeRequestApproveListener.class);
	...
	public void onMessage(Message message) {
		if (message instanceof ObjectMessage) {
			...
			try {
				...
				try {
					changeRequestProcessInternalService.approveChangeRequestTask(changeRequest, changeRequestMessage.getUserDetails(), salesPosition);
