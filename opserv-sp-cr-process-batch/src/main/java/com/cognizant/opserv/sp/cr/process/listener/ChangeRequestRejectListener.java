 ...
 *        ***************
 */
@Component("changeRequestRejectListener")
@EnableTransactionManagement
public class ChangeRequestRejectListener implements MessageListener {

	@Autowired
	private ChangeRequestOfflineService changeRequestProcessInternalService;

	private static final OpservLogger LOGGER = OpservLoggerFactory.getOpservLogger(ChangeRequestRejectListener.class);
	...
	public void onMessage(Message message) {
		if (message instanceof ObjectMessage) {
			...
			try {
				...
				try {
					changeRequestProcessInternalService.rejectChangeRequestTask(changeRequest, changeRequestMessage.getUserDetails(), salesPosition);
