 ...
 *        ***************
 */
@Component("changeRequestApproveWorkflowListener")
@EnableTransactionManagement
public class ChangeRequestApproveWorkFlowListener implements MessageListener {

	@Autowired
	private ChangeRequestOfflineService changeRequestProcessInternalService;

	private static final OpservLogger LOGGER = OpservLoggerFactory.getOpservLogger(ChangeRequestApproveWorkFlowListener.class);
	...
	public void onMessage(Message message) {
		if (message instanceof ObjectMessage) {
			...
			try {
				...
				try {
					changeRequestProcessInternalService.approveChangeRequest(changeRequest, userDetails, salesPosition);
