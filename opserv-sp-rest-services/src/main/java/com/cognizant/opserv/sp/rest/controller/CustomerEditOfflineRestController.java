 ...
 * ****************************************************************************.
 *
 * @class CustomerEditOfflineRestController contains to call the rest services for CustomerEditOfflineService
 ...
 */
@Controller
public class CustomerEditOfflineRestController {
	
	private static final OpservLogger LOGGER = OpservLoggerFactory.getOpservLogger(CustomerEditOfflineRestController.class);


	@Autowired
	CustomerEditOfflineService customerEditOfflineService;
	...
	@RequestMapping(value="/customerEditOffline/validate/" +
	...
	public @ResponseBody ServiceResponse<List<CustomerProductAlignment>> validateCustomerAlignmentDTO(
			...
			@PathVariable("userId") Integer userId,		
				HttpServletRequest request) throws SalesPositionServiceException {
		...
		try {
			...
			try {
				customerEditOfflineService.validate(customerAlignmentDTO, userDetails);
	...
	@RequestMapping(value = "/CustomerEditOffline/createMirrorCR/{salespoId}/{buId}/{salesTeamId}/{almntId}/{hierarchyId}/{salespoTrgId}/{hierarchyTrgId}", method = RequestMethod.GET, produces = "application/json")
	public @ResponseBody
	ServiceResponse<Boolean> createMirrorCR(
		...
		try {
			...
			customerEditOfflineService.createMirrorCR(parentCR1, mirrorDTO,userDts);
	...
	@RequestMapping(value="/CustomerEditOffline/getMirrorCRsForEdit/{chngreqId}", method=RequestMethod.GET, produces="application/json")	public @ResponseBody
	ServiceResponse<Boolean> getMirrorCRsForEdit(@PathVariable("chngreqId") Long chngreqId, HttpServletRequest request) throws ChangeRequestServiceException {
		ServiceStatus serviceStatus = null;
		ServiceResponse serviceResponse = new ServiceResponse();
		ServiceResult serviceResult = new ServiceResult();
		try {
		...
		customerEditOfflineService.getMirrorCRsForEdit(custAlgmtDTO,  ModelAssembler.getDefaultUserDetails());
		serviceStatus = new ServiceStatus(StatusCode.OK, "200 OK");
		} catch (Exception e) {
			serviceResult.setDetail("Exception occured "+e.getMessage());
			serviceStatus = new ServiceStatus(StatusCode.SERVER_ERROR, "500 ERROR");
	...
	@RequestMapping(value="/CustomerEditOffline/updateChangeRequests/{chngreqId}", method=RequestMethod.GET, produces="application/json")
	public @ResponseBody
	ServiceResponse<Boolean> updateChangeRequests(@PathVariable("chngreqId") Long chngreqId, HttpServletRequest request)
			throws ChangeRequestServiceException, AlignmentServiceException {
		...
		customerEditOfflineService.updateChangeRequests(customerAlgmtDTO, ModelAssembler.getDefaultUserDetails());
		serviceStatus = new ServiceStatus(StatusCode.OK, "200 OK");
			} catch (Exception e) {
				serviceResult.setDetail("Exception occured "+e.getMessage());
				serviceStatus = new ServiceStatus(StatusCode.SERVER_ERROR, "500 ERROR");
