@echo off
rem This script runs SP Trigger Stored Proc Batch
...
echo. 
echo Starting SP Trigger Stored Proc Batch....

rem Set Java Opts
set "JAVA_OPTS=-Xms256m -Xmx512m"
set "JAVA_OPTS=%JAVA_OPTS% -DappType=standalone -DtargetDataSource=app -DappName=SpTriggerSPBatch"
...
set JAVA_CP=%BATCH_DIR%opserv-sp-trigger-eod-sp.jar;%JAVA_CP%;
...
rem set JAVA_OPTS=%JAVA_OPTS% -Dlog4j.configurationFile=file:///%OPSERV_HOME%/config/opserv-sp-trigger-eod-sp-log-config.xml
...
set command=%JAVA_CMD% %JAVA_OPTS% -cp %JAVA_CP% com.cognizant.opserv.sp.TriggerStoredProc
echo COMMAND USED : %command%
echo.
"%JAVA_CMD%" %JAVA_OPTS% -cp "%JAVA_CP%" com.cognizant.opserv.sp.TriggerStoredProc
