...
public class CustomerServiceTest extends BaseTest{
	...
	public void searchCustomersByCriteriaTest() throws CustomerServiceException{
		...
		IExpressionCriteria custName = ExpressionCriteria.createLikeCriteria("customer.custName","%HOSPITAL%");

		IQuery query = new Query().criteria(custName);
		PaginationInfo pagination = new PaginationInfo(0, 10);
		query.pagination(pagination);
		List<Customer> customerList = customerService.searchCustomers(query, userDetails);
	...
	public void getCustomerCountByCriteriaTest() throws CustomerServiceException{
		...
		IExpressionCriteria custName = ExpressionCriteria.createLikeCriteria("customer.custName","%HOSPITAL%");
		IExpressionCriteria custType = ExpressionCriteria.createEqualToCriteria("customer.custTypeId", 3);
		
		IQuery query = new Query().criteria(custName.and(custType));
		Long customerCount = customerService.getCustomerCount(query, userDetails);
